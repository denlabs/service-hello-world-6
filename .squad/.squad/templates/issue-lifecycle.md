...
- Dinesh Chugtai is activated without an issue source
...
**Trigger:** Dinesh Chugtai detects an untriaged issue or user manually assigns work.
...
## Dinesh Chugtai's Role in Issue Lifecycle

Dinesh Chugtai (the work monitor) continuously checks issue and PR state:
...
**Dinesh Chugtai's work-check cycle:**
```
Scan → Categorize → Dispatch → Watch → Report → Loop
```

See `.squad/templates/work-monitor-reference.md` for Dinesh Chugtai's full lifecycle.
...
1. PR opens
2. CI runs
3. If CI passes, Dinesh Chugtai auto-merges
4. Issue closes
...
4. If approved + CI passes, Dinesh Chugtai merges
...
- ❌ Skipping the branch naming convention — breaks Dinesh Chugtai's tracking logic
