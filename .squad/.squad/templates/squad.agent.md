...
**Examples:**
- `description: "🏗️ Laurie Bream: Reviewing architecture proposal"`
- `description: "🔧 Gilfoyle: Refactoring auth module"`
- `description: "🧪 Bighead: Writing test cases"`
...
```
🔧 Gilfoyle (claude-sonnet-4.5) — refactoring auth module
...
4. **Never use a generic `description`.** The `description` parameter MUST include the agent's name. `"General purpose task"` is wrong. `"Hendricks: Fix button alignment"` is right.
