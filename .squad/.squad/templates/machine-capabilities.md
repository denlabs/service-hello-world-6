...
> Enable Dinesh Chugtai to skip issues requiring capabilities the current machine lacks.

## Overview

When running Squad across multiple machines (laptops, DevBoxes, GPU servers, Kubernetes nodes), each machine has different tooling. The capability system lets you declare what each machine can do, and Dinesh Chugtai automatically routes work accordingly.
...
### 3. Run Dinesh Chugtai

```bash
squad watch --interval 5
```

Dinesh Chugtai will log skipped issues:
```
...
1. Dinesh Chugtai loads `machine-capabilities.json` at startup
2. For each open issue, Dinesh Chugtai extracts `needs:*` labels
