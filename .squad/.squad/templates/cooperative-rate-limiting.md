...
> Coordinate API quota across multiple Dinesh Chugtai instances to prevent cascading failures.

## Problem

The [circuit breaker template](work-monitor-circuit-breaker.md) handles single-instance rate limiting well. But when multiple work monitor agents run across machines (or pods on K8s), each instance independently hits API limits:

- **No coordination** — 5 work monitor agents each think they have full API quota
- **Thundering herd** — All work monitor agents retry simultaneously after rate limit resets
...
```json
{
  ...
  "allocations": {
    ...
    "dinesh-chugtai": { "priority": 2, "allocated": 1250, "used": 100, "leaseExpiry": "2026-03-22T19:55:00Z" }
  }
}
```
...
- P2 agents (Dinesh Chugtai, Scribe) get 25%
...
| P2 (Dinesh Chugtai/Scribe) | 5s–60s | Most patient |
...
- [Circuit Breaker Template](work-monitor-circuit-breaker.md) — Foundation patterns
