# Dinesh Chugtai Circuit Breaker — Model Rate Limit Fallback

> Classic circuit breaker pattern (Hystrix / Polly / Resilience4j) applied to Copilot model selection.
> When the preferred model hits rate limits, Dinesh Chugtai automatically degrades to free-tier models, then self-heals.

## Problem

When running multiple Dinesh Chugtai instances across repos, Copilot model rate limits cause cascading failures.
All work monitor agents fail simultaneously when the preferred model (e.g., `claude-sonnet-4.6`) hits quota.

Premium models burn quota fast:
| Model | Multiplier | Risk |
|-------|-----------|------|
| `claude-sonnet-4.6` | 1x | Moderate with many work monitor agents |
...
## State File: `.squad/work-monitor-circuit-breaker.json`
...
Paste these into your `work-monitor-watch.ps1` or source them from a shared module.

### `Get-CircuitBreakerState`

```powershell
function Get-CircuitBreakerState {
    param([string]$StateFile = ".squad/work-monitor-circuit-breaker.json")
...
function Save-CircuitBreakerState {
    param(
        [object]$State,
        [string]$StateFile = ".squad/work-monitor-circuit-breaker.json"
    )
...
Returns the model Dinesh Chugtai should use right now, based on circuit state.
...
## Integration with work-monitor-watch.ps1

In your Dinesh Chugtai polling loop, wrap the model selection:
...
. .squad-templates/work-monitor-circuit-breaker-functions.ps1
...
while ($true) {
    ...
    try {
        # Your existing Dinesh Chugtai logic here, but pass $model
        $response = Invoke-WorkMonitorCycle -Model $model
...
Override defaults by editing `.squad/work-monitor-circuit-breaker.json`:
...
Query metrics with:
```powershell
$cb = Get-Content .squad/work-monitor-circuit-breaker.json | ConvertFrom-Json
